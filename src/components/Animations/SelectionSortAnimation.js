...
const SelectionSortVisualizer = () => {
  ...
  const [speed, setSpeed] = useState(2); // Slower default speed
  ...
  const startSelectionSort = async () => {
    ...
    for (let i = 0; i < arr.length - 1 && sortingActiveRef.current; i++) {
      let minIdx = i;
      setCurrentStep(`Pass ${i + 1}: Finding minimum element`);
      setActiveIndices([i, minIdx]);
      await delay(1000 / speed);
      if (!sortingActiveRef.current) break;

      // Find the minimum element
      for (let j = i + 1; j < arr.length && sortingActiveRef.current; j++) {
        setCurrentStep(`Comparing ${arr[j]} and ${arr[minIdx]}`);
  ...
  <div className="min-h-full bg-gray-100 p-1 sm:p-16 md:p-8">
      <div className="max-w-6xl mx-auto w-full p-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-center mb-4 sm:mb-8">Selection Sort Visualizer</h1>
